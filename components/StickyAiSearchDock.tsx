...
export function StickyAiSearchDock() {
  ...
  const handleExpand = (e: React.FormEvent | React.MouseEvent) => {
    e.preventDefault();
    setIsExpanded(true);
  };
  ...
  return (
    <>
      {!isExpanded && (
        <div className="sticky-ai-dock-container" onClick={handleExpand}>
          <form className="hero__search is-sticky" onSubmit={handleExpand}>
      ...
      {/* Expands in place where the pill was — no modal, no backdrop. */}
      <AiQueryPanel
        isOpen={isExpanded}
